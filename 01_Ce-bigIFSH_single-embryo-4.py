...
# 250312 - ok so notebook is parsing all variables from bash script as expected now. The bug is in the output directory. Making empty output_directory (with proper subdirectories, also empty). Output is being saved inside the input_directory in a "_results" folder. Goal for today: redirect output. 
# 
# --> ok actually, last I did was to remove the _results directory. That put me inside the input dir where the image files are. Will reuse the _results dir to maintain the file hierarchy. will up one more and call output. 
# 
# 
# --> input: ok so, for running as. jupyter notebook we can call the image_subdirecotry "input_directory" directly because there is only one image. 
# When running as array we should copy the path to the folder_name (local_download_directory) on script, so we can create output_directories for each, since we are now working with multiple images in parallel.
...
# # # Note: input directory = image subdiorectory
# input_directory = '/pl/active/onishimura_lab/PROJECTS/naly/bigfish/02_DG_quantification/N2_chs-1_set-3/250207_N2_chs-1/2-cell/input/input/14' #your folder should contain 2 images (.dv and its ref.dv)
# folder_name = input_directory

# # User-defined channel names (set to None if the channel does not exist)
# Cy5 = "set-3_mRNA"  # (Q670)
# mCherry = "chs-1_mRNA"  # (Q610)
# FITC = "nothing"  # (GFP)
# DAPI = "DAPI"
...
# # input_directory = os.path.join(folder_name, 'input')
...
if run_mRNA_heatmaps:
    def create_heatmap(spots_x, spots_y, rna_projection, masks_cytosol, masks_nuclei, output_filename, title_suffix, grid_width=80, grid_height=80):
        # Calculate the width and height of each grid cell
        img_width, img_height = rna_projection.shape[1], rna_projection.shape[0]
        cell_width = img_width / grid_width
        cell_height = img_height / grid_height
        ...
        # Create a heatmap using the correct RNA projection
        plt.imshow(rna_projection, cmap='gray')  # Show the RNA max projection
        plt.imshow(masks_cytosol, cmap='gray', alpha=0.3)  # Overlay cytosol mask with transparency
        plt.imshow(masks_nuclei, cmap='gray', alpha=0.3)   # Overlay nuclei mask with transparency
        plt.imshow(grid, cmap='CMRmap', alpha=1, interpolation='nearest')  # Overlay the heatmap
        plt.title(f'mRNA Abundance Heatmap ({title_suffix})')
        plt.axis('off')
    ...
    # Generate heatmaps for each RNA channel with the correct max projection
    if 'spots_post_clustering_ch0' in locals():
        spots_x_ch0 = spots_post_clustering_ch0[:, 2]
        spots_y_ch0 = spots_post_clustering_ch0[:, 1]
        cy5_heatmap = create_heatmap(spots_x_ch0, spots_y_ch0, image_Cy5, masks_cytosol, masks_nuclei, os.path.join(output_directory, f'{Cy5}_heatmap.png'), Cy5)

    if 'spots_post_clustering_ch1' in locals():
        spots_x_ch1 = spots_post_clustering_ch1[:, 2]
        spots_y_ch1 = spots_post_clustering_ch1[:, 1]
        mcherry_heatmap = create_heatmap(spots_x_ch1, spots_y_ch1, image_mCherry, masks_cytosol, masks_nuclei, os.path.join(output_directory, f'{mCherry}_heatmap.png'), mCherry)
